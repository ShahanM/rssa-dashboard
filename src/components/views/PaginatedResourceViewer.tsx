...
const PaginatedResourceViewer = <T extends { id: string }>({
	...
	const { api } = useApi<PaginatedDataList<T>>();

	const fetchResources = async (page: number) => {
		const offset = page * limit;
		return api.get(`${apiResourceTag}/?offset=${offset}&limit=${limit}`);
	};
	...
	if (isLoading && !data) {
		return <div className="flex justify-center items-center h-64"><p className="text-gray-500">Loading {apiResourceTag}...</p></div>;
	}
	...
	return (
		<div className="flex flex-col h-full">
			...
			<div className="flex-shrink-0 p-4 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 flex items-center justify-center">
				<button
